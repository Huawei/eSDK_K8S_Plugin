...
// NFSPlus to define a local lock when connect or disconnect, in order to preventing mounting and unmounting confusion
type NFSPlus struct {
}

func init() {
	connector.RegisterConnector(connector.NFSPlusDriver, &NFSPlus{})
}
...
func (nfsPlus *NFSPlus) ConnectVolume(ctx context.Context, conn map[string]interface{}) (string, error) {
...
func (nfsPlus *NFSPlus) DisConnectVolume(ctx context.Context, targetPath string) error {
