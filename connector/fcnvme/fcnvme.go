...
// Package fcnvme provide the way to connect/disconnect volume within FC NVMe protocol
package fcnvme
...
// Conn implements the connector.VolumeConnector for fc-nvme protocol
type Conn struct {
	mutex sync.Mutex
}

func init() {
	connector.RegisterConnector(connector.FCNVMeDriver, &Conn{})
}
...
func (fc *Conn) ConnectVolume(ctx context.Context, conn map[string]interface{}) (string, error) {
...
func (fc *Conn) DisConnectVolume(ctx context.Context, tgtLunGuid string) error {
