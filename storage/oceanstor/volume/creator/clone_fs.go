...
func (creator *CloneFsCreator) CreateVolume(ctx context.Context) (utils.Volume, error) {
	...
	if fs != nil {
		if err := creator.waitFsSplit(ctx, getValueOrFallback(fs, "ID", "")); err != nil {
			return nil, err
		}

		volume.SetID(getValueOrFallback(fs, "ID", ""))
		return volume, nil
	}
...
func (creator *CloneFsCreator) clone(ctx context.Context) (string, error) {
	...
	srcFsCapacity, err := strconv.ParseInt(
		getValueOrFallback(cloneFromFS, "CAPACITY", ""),
		capacityBase,
		capacityBitSize,
	)
	if err != nil {
	...
	cloneFilesystemReq := &cloneFilesystemRequest{
		...
		parentID:             getValueOrFallback(cloneFromFS, "ID", ""),
...
func (creator *CloneFsCreator) cloneFilesystem(ctx context.Context, req *cloneFilesystemRequest) (string, error) {
	...
	cloneFSID := getValueOrFallback(cloneFS, "ID", "")
	...
	req.vStoreId = getValueOrFallback(cloneFS, "vstoreId", systemVStore)
