...
func NewSAN(cli, metroRemoteCli, replicaRemoteCli client.OceanstorClientInterface,
	product constants.OceanstorVersion) *SAN {
	return &SAN{
		Base: Base{
			cli:              cli,
...
func (p *SAN) Query(ctx context.Context, name string, params map[string]interface{}) (utils.Volume, error) {
	...
	if err = p.validateManageWorkLoadType(ctx, params, lun); err != nil {
		return nil, err
	}
...
func (p *SAN) validateManageWorkLoadType(ctx context.Context, params, lun map[string]interface{}) error {
	err := p.setWorkLoadID(ctx, p.cli, params)
	if err != nil {
		return err
	}
	...
	oldWorkloadTypeID, ok := utils.GetValue[string](lun, "WORKLOADTYPEID")
	if !ok {
		return nil
	}
...
func (p *SAN) createRemoteLun(ctx context.Context,
	...
	if !ok {
		return nil, pkgUtils.Errorf(ctx,
			"remoteCli convert to client.OceanstorClientInterface failed, data: %v", taskResult["remoteCli"])
...
func (p *SAN) preExpandCheckRemoteCapacity(ctx context.Context,
	params map[string]interface{}, cli client.OceanstorClientInterface) (string, error) {
...
func (p *SAN) deleteLun(ctx context.Context, name string, cli client.OceanstorClientInterface) error {
