...
func TestParameters_Product(t *testing.T) {
	// arrange
	want := constants.OceanstorVersion("DoradoV6")
...
func TestParameters_ProductIsV6(t *testing.T) {
	...
	// act
	got := params.Product().IsDoradoV6()

	// assert
	require.True(t, got)
}

func TestParameter_AdvancedOptions(t *testing.T) {
	// arrange
	value := `{"CAPACITYTHRESHOLD": 90}`
	in := map[string]any{constants.AdvancedOptionsKey: value}
	params := creator.NewParameter(in)
	...
	// assert
	require.Equal(t, value, advancedOptions)
}
