...
func (creator *ModifyFsCreator) CreateVolume(ctx context.Context) (utils.Volume, error) {
	...
	if creator.hyperMetro {
		if err = creator.modifyHyperMetro(ctx, activeFs); err != nil {
			return nil, err
		}
	}
...
func (creator *ModifyFsCreator) modifyHyperMetro(ctx context.Context, activeFs map[string]any) error {
	// modify local filesystem to hyper metro filesystem
	pairId, err := creator.getPairIdByFsId(ctx, activeFs)
	if err != nil {
		return fmt.Errorf("failed to get pair id of active hypermetro pair, error: %v", err)
	}
	if pairId != "" {
	...
	creator.transaction.Then(func() error {
		standbyCreator.setStandbyParameters(activeFs)
		...
		pairId, err = creator.createHyperMetroPair(ctx, utils.GetValueOrFallback(activeFs, "ID", ""), standbyFsId)
		return err
	}, func() {
...
func (creator *ModifyFsCreator) newStandbyCreatorForModify(ctx context.Context,
	...
	poolName := utils.GetValueOrFallback(activeFs, "PARENTNAME", "")
	if poolName == "" {
	...
	standbyCreator.description = utils.GetValueOrFallback(activeFs, "DESCRIPTION", "")
