...
func TestFilterByStorageQuota(t *testing.T) {
	tests := []struct {
		...
		{name: "NormalSoftQuota", storageQuota: `{"spaceQuota": "softQuota", "gracePeriod": 100}`,
			candidatePools: []*model.StoragePool{{Capabilities: map[string]bool{"SupportQuota": true}}},
			expect:         1, expectErr: false},
		{name: "NormalHardQuota", storageQuota: `{"spaceQuota": "hardQuota"}`,
			candidatePools: []*model.StoragePool{{Capabilities: map[string]bool{"SupportQuota": true}}},
			expect:         1, expectErr: false},
		{name: "NegativePeriod", storageQuota: `{"spaceQuota": "hardQuota", "gracePeriod": -1}`,
			candidatePools: []*model.StoragePool{{Capabilities: map[string]bool{"SupportQuota": true}}},
			expect:         0, expectErr: true},
		{name: "ExceedsTheMaximumPeriod", storageQuota: `{"spaceQuota": "hardQuota", "gracePeriod": 4294967295}`,
			candidatePools: []*model.StoragePool{{Capabilities: map[string]bool{"SupportQuota": true}}},
			expect:         0, expectErr: true},
		{name: "WrongType", storageQuota: `{"spaceQuota": "WrongType"`,
			candidatePools: []*model.StoragePool{{Capabilities: map[string]bool{"SupportQuota": true}}},
			expect:         0, expectErr: true},
		{name: "HardWithPeriod", storageQuota: `{"spaceQuota": "hardQuota", "gracePeriod": 10}`,
			candidatePools: []*model.StoragePool{{Capabilities: map[string]bool{"SupportQuota": true}}},
			expect:         0, expectErr: true},
	}
