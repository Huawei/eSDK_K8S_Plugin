...
func TestControllerGetVolume_OceanstorSan_Success_NormalStatus(t *testing.T) {
	// Arrange
	...
	cli.EXPECT().GetLunByName(gomock.Any(), data.LunName).Return(
...
func TestControllerGetVolume_OceanstorSan_FaultStatus(t *testing.T) {
	// Arrange
	data := fakeOceanstorSanData()
	ctx := context.Background()
	mockCtrl := gomock.NewController(t)
...
func TestControllerGetVolume_OceanstorSan_VolumeNotFound(t *testing.T) {
	// Arrange
	data := fakeOceanstorSanData()
	ctx := context.Background()
	mockCtrl := gomock.NewController(t)
	...
	cli.EXPECT().GetLunByName(gomock.Any(), data.LunName).Return(nil, nil)
	cli.EXPECT().Logout(ctx)
...
func TestControllerGetVolume_OceanstorSan_APIError(t *testing.T) {
	// Arrange
	data := fakeOceanstorSanData()
	ctx := context.Background()
	mockCtrl := gomock.NewController(t)
...
func fakeOceanstorSanData() *oceanstorSanData {
	return &oceanstorSanData{
		BackendName:  "san-backend",
		LunName:      "test-lun",
		VolumeId:     "san-backend.test-lun",
		HealthStatus: "1",
	}
}

type oceanstorSanData struct {
	BackendName  string
	LunName      string
	VolumeId     string
	HealthStatus string
...
func (d *oceanstorSanData) request() *csi.ControllerGetVolumeRequest {
	return &csi.ControllerGetVolumeRequest{VolumeId: d.VolumeId}
}

func (d *oceanstorSanData) backend(cli client.OceanstorClientInterface) model.Backend {
