...
func newLogger(conf map[string]string) (*Logger, error) {
	...
	if "" != conf["logFileMaxCap"] {
		...
		if err != nil {
			glog.Errorf("Calc max log file size error: %v.", err)
			return nil, err
		}
	...
	if err != nil {
		glog.Errorf("Obtain log file error: %v.", err)
		return nil, err
	}
...
func (l *Logger) writeLogMsg(level int, logMsg string) {
	...
	if err != nil {
		glog.Errorf("Write log message %s to %s error.", logContent, l.logFilePath)
	}
}
